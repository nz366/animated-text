...
use super::model::{AnimationData, Keyframe, LyricLine};
...
impl App {
    ...
    fn update(&mut self) {
        ...
        if !self.manual_scroll {
            if let Some(idx) = self.get_active_line_index() {
                ...
                let closest = self.data.lines.iter()
    ...
    fn handle_keyframe_editor_keys(&mut self, code: KeyCode) {
        let Some(idx) = self.focus_line_index.or(self.get_active_line_index()) else { return; };
        ...
        match code {
            ...
            KeyCode::Char('k') => {
                ...
                if let Some((i, kf)) = line.keyframes.iter().enumerate().find(|(_, k)| k.time > rel_time + 0.01) {
                ...
                if idx + 1 < self.data.lines.len() {
                    ...
                    self.current_time = self.data.lines[next_idx].start;
            ...
            KeyCode::Char('j') => {
                let line = &self.data.lines[idx];
                let rel_time = self.current_time - line.start;
                if let Some((i, kf)) = line.keyframes.iter().enumerate().rev().find(|(_, k)| k.time < rel_time - 0.01) {
                    self.active_kf_index = Some(i);
                    self.current_time = line.start + kf.time;
                    return;
                }
                if idx > 0 {
                    ...
                    self.current_time = self.data.lines[prev_idx].start;
                    self.active_kf_index = Some(self.data.lines[prev_idx].keyframes.len().saturating_sub(1));
                }
                self.is_playing = false;
            }
            _ => {}
    ...
    fn find_closest_kf_idx(&self, line_idx: usize, rel_time: f32) -> Option<usize> {
        self.data.lines[line_idx].keyframes.iter().enumerate()
            .min_by(|(_, a), (_, b)| (a.time - rel_time).abs().partial_cmp(&(b.time - rel_time).abs()).unwrap())
            .map(|(i, _)| i)
    }
    ...
    fn handle_text_edits(&mut self, key: KeyEvent) {
        ...
        let line_idx = if let Some(i) = self.focus_line_index { i } else { return; };
        ...
        match key.code {
            ...
            KeyCode::Right => {
                if self.cursor_col < self.data.lines[line_idx].text.chars().count() {
                    self.cursor_col += 1;
                }
            }
            KeyCode::Up => {
            ...
            KeyCode::Down => {
                ...
                } else if line_idx + 1 < self.data.lines.len() {
                    self.focus_line_index = Some(line_idx + 1);
                    let new_len = self.data.lines[line_idx + 1].text.chars().count();
                    self.cursor_col = self.cursor_col.min(new_len);
                }
...
impl UI {
    ...
    fn render_header(f: &mut Frame, app: &App, area: Rect) {
        let mode_str = match app.view_mode {
            ViewMode::List => "LINE MODE [ESC] | TEXT EDIT [E] | [Q] Quit | [SPACE] Play",
        ...
        let rel_time = app.focus_line_index
        ...
        let info = format!(" {} | Time: {:.2}s |  Relative: {:.2}s ", mode_str, app.current_time, rel_time);
        ...
        let p = Paragraph::new(vec![TuiLine::from(info), TuiLine::from(sub_info)]).block(
            Block::default().borders(Borders::ALL).border_style(Style::default().fg(status_color)),
        );
    ...
    fn render_focus_mode(f: &mut Frame, app: &App, area: Rect) {
        ...
        if let Some(idx) = active_idx {
            ...
            f.render_widget(Paragraph::new("Waiting for next line...").alignment(Alignment::Center), chunks[0]);
        }
    }

    fn get_animated_line_spans<'a>(line: &'a LyricLine, current_time: f32, is_active: bool) -> Vec<Span<'a>> {
        let rel_time = current_time - line.start;
        let target_idx = line.get_current_index(rel_time);

        line.text.chars().enumerate().map(|(i, c)| {
    ...
    fn render_active_line_anim(f: &mut Frame, app: &App, idx: usize, area: Rect) {
        ...
        f.render_widget(Paragraph::new(TuiLine::from(spans)).alignment(Alignment::Center).block(Block::default().borders(Borders::NONE)), area);
    }
    ...
    fn render_list_mode(f: &mut Frame, app: &App, area: Rect) {
        ...
        for (i, lyric) in app.data.lines.iter().enumerate() {
            ...
            let mut line_spans = vec![
                Span::styled(format!("[{:.2}] ", lyric.start), Style::default().fg(Color::Gray)),
                Span::styled(prefix, if is_playing { Style::default().fg(Color::Green) } else { Style::default().fg(Color::Blue) }),
            ];

            if is_editing {
                let text_chars: Vec<char> = lyric.text.chars().collect();
                for (char_idx, c) in text_chars.iter().enumerate() {
                    let char_style = if char_idx == app.cursor_col { Style::default().bg(Color::Blue).fg(Color::White) } else { Style::default().fg(Color::White) };
                ...
                line_spans.extend(Self::get_animated_line_spans(lyric, app.current_time, is_playing));
            } else {
                let style = if is_selected { Style::default().fg(Color::Blue).add_modifier(Modifier::REVERSED) } else { Style::default().fg(Color::DarkGray) };
                line_spans.push(Span::styled(lyric.text.clone(), style));
            }
            tui_lines.push(TuiLine::from(line_spans));
        }

        let scroll_target = if is_text_editor { app.focus_line_index.unwrap_or(display_idx) } else { display_idx };
        let scroll_pos = if scroll_target > 5 { (scroll_target - 5) as u16 } else { 0 };
        let title = if is_text_editor { " Exit Edit [ESC] " } else { " [E] EDIT " };

        f.render_widget(Paragraph::new(tui_lines).alignment(Alignment::Left).scroll((scroll_pos, 0)).block(Block::default().borders(Borders::ALL).title(title)), area);
    }
    ...
    fn render_keyframe_editor_panel(f: &mut Frame, app: &App, idx: usize, area: Rect) {
        ...
        let kfs = line.keyframes.iter().enumerate().flat_map(|(ki, k)| {
            let is_near = if let Some(aki) = app.active_kf_index { ki == aki } else { (k.time - rel_time).abs() < 0.1 };
            let base_fg = if is_near { Color::Yellow } else { Color::DarkGray };
            let time_style = if app.edit_mode == EditMode::Time && is_near { Style::default().bg(Color::Blue).fg(Color::White) } else { Style::default().fg(base_fg) };
            let progress_style = if app.edit_mode == EditMode::Progress && is_near { Style::default().bg(Color::Blue).fg(Color::White) } else { Style::default().fg(base_fg) };
            vec![
                ...
                Span::styled(format!("{:.0}%", (k.index / line.text.len().max(1) as f32) * 100.0), progress_style),
                Span::styled("] ", Style::default().fg(base_fg)),
            ]
        }).collect::<Vec<Span>>();
        ...
        f.render_widget(Paragraph::new(ui_info).block(Block::default().borders(Borders::TOP).title("KeyFrame Editor")).alignment(Alignment::Center), area);
    }
}

// --- MAIN RUN FUNCTION ---

pub fn run_app() -> io::Result<String> {
    enable_raw_mode()?;
